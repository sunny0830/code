...
x_train_sigmoidal = np.zeros((17, 766))
for i in range(766):
...
##############################
x_test_sigmoidal = np.zeros((17, 330))
for i in range(330):
...
weight = w.T
x_train_predict = np.dot(weight, x_train_sigmoidal)
# 計算MSE和RMSE
...
plt.title('Sigmoidal(train)', fontsize=12)
...
# 對驗證數據集的比較
plt.rcParams["figure.figsize"] = (18,8) # 顯示圖像的最大範圍
plt.title('Sigmoidal(test)', fontsize=12)
plt.xlabel('The nth data', fontsize=10)
plt.ylabel('PM2.5', rotation = 90, fontsize=10)
