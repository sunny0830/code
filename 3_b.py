...
def linear_regression(S,x,y):
    w = np.matmul(np.matmul(beta*np.linalg.pinv(S),x.T),y)
    return w
...
for i in range(9):
    ...
    x_train_gaussian = np.zeros((17, 975))
    for i in range(975):
    ...
    ##############################
    x_test_gaussian = np.zeros((17, 121))
    for i in range(121):
    ...
    x = x_train_gaussian.T # 766*18
    y= test_training_array.T # 766*1
    # np.eye 生成單位矩陣
    I = np.eye(18, dtype = int)
    alpha = 5
    beta = 10
    S = np.linalg.pinv(alpha*I)+beta*(np.matmul(x.T,x))
    w = linear_regression(S,x,y)
    weight = w.T
...
for i in range(9):
    ...
    x_train_gaussian = np.zeros((5, 975))
    for i in range(975):
        training_array_i = training_array[:,i]
        for j in range(5):
            training_j = training_array_i[j] - x_mean[j]
    ...
    ##############################
    x_test_gaussian = np.zeros((5, 121))
    for i in range(121):
        testing_array_i = testing_array[:,i]
        for j in range(5):
            testing_j = testing_array_i[j] - x_mean[j]
...
for i in range(9):
    ...
    x_train_gaussian = np.zeros((12, 975))
    for i in range(975):
        training_array_i = training_array[:,i]
        for j in range(12):
            training_j = training_array_i[j] - x_mean[j]
    ...
    ##############################
    x_test_gaussian = np.zeros((12, 121))
    for i in range(121):
        testing_array_i = testing_array[:,i]
        for j in range(12):
            testing_j = testing_array_i[j] - x_mean[j]
